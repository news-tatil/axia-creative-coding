...
class UI_window {
        ...
        for (let section of this.sections) {
            ...
            for (let item of items) {
                ...
                if (item.type === "gridselector") {
                    component
                        .querySelectorAll(".colored, .image")
                        .forEach((opt) => (opt.style.height = `${opt.offsetWidth}px`))
    ...
    create_component(component_obj) {
        ...
        switch (component_obj.type) {
            ...
            case "range_slider":
                ...
                if (component_obj.min === undefined) component_obj.min = 0
                if (component_obj.max === undefined) component_obj.max = 100
                if (component_obj.start === undefined)
                    component_obj.start = Math.round((component_obj.min + component_obj.max) / 3)
                if (component_obj.end === undefined)
                    component_obj.end = Math.round(((component_obj.min + component_obj.max) * 2) / 3)

                input = document.createElement("input")
                input.kind = "range_slider"
                input.name = component_obj.name
                input.id = "input_" + component_obj.name
            ...
            case "color":
                component = document.createElement("div")
                component.className = "ui input color"
                component.id = "color_" + component_obj.name

                label = document.createElement("label")
                label.innerText = component_obj.text

                let floater___ = document.createElement("div")
                floater___.className = "input_floater"
                ...
                floater___.append(input)
                // if (component_obj.steps) floater__.append(b_add);
                component.append(label)
                component.append(floater___)
                // component.append(input);
    ...
    refresh() {
        for (let prop in this.data) {
            ...
            if (input.kind === "range_slider") {
                let [start, end] = this.data[prop].split(",").map(Number)
                const sliderElement = input.parentElement // The parent element is the slider container
                const currentValues = [
                    $(sliderElement).slider("get thumbValue", "first"),
                    $(sliderElement).slider("get thumbValue", "second"),
                ]

                // Check if the slider values need to be updated
                if (currentValues[0] !== start || currentValues[1] !== end) {
                    $(sliderElement).slider("set rangeValue", start, end) // Update the slider values
                    // changed = true;
                }
            } else if (input.type === "checkbox" && input.checked != this.data[prop]) {
                input.checked = this.data[prop]
                changed = true
            } else if (input.type === "text" && input.value != this.data[prop]) {
